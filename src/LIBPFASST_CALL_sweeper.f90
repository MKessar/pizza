...
!      type(pf_fft_t), pointer :: fft_tool
     ...
     complex(pfdp), allocatable :: theta_Rloc(:,:) !u_phi velocity in complex space
     ...
     real(pfdp), allocatable :: theta_r_Rloc(:,:) !u_phi velocity in real space
...
!     nRstop   = n_r_max !nRstop_lvl(level_index)
    ...
    allocate(this%theta_Rloc   (n_m_max,nRstart:nRstop))
...
!  print*,"Barrier a init Key_Pizza =",Key_Pizza 
!     call mpi_barrier(Comm_Pizza, ierror)
! 
!  print*,"Barrier b init Key_Pizza =",Key_Pizza 
...
!   print*,"Barrier c init Key_Pizza =",Key_Pizza 
!     call mpi_barrier(Comm_Pizza, ierror)

!  print*,"Barrier d init Key_Pizza =",Key_Pizza  
    
     
    call transp_m2r(m2r_fields, this%u_s_m_Mloc, this%u_s_m_Rloc)

    call transp_m2r(m2r_fields, this%u_phi_m_Mloc, this%u_phi_m_Rloc)
!     print*,"Barrier e init Key_Pizza =",Key_Pizza 
!     call mpi_barrier(Comm_Pizza, ierror)

!      print*,"Barrier f init Key_Pizza =",Key_Pizza     
...
!     call transp_r2m(r2m_fields,  this%u_phi_m_Rloc, this%tmphat_Mloc)
! 
!         this%tmphat_Mloc=this%u_phi_m_Mloc-this%tmphat_Mloc
!         error=maxval(abs(real(this%tmphat_Mloc)))
!         print*,"error =",error,"Key_Pizza =",Key_Pizza
...
!     theta_Mloc  => get_array2d(y)
    theta  => get_array2d(y)
    fvec => get_array2d(f)
    this%fvec_Mloc=0.0_8
    
!     do n_r=1,n_r_max
!        do n_m=nMstart,nMstop
! !           this%fvec_Mloc(n_m,n_r) =fvec (n_m,n_r)
!           this%theta_Mloc(n_m,n_r) =theta (n_m,n_r)
! !           this%rhsvec_Mloc(n_m,n_r) =rhsvec (n_m,n_r)
!                                
!        enddo
!     enddo
!     
...
!        do n_m=nMstart,nMstop
! !           this%fvec_Mloc(n_m,n_r) =fvec (n_m,n_r)
!           this%theta_Mloc(n_m,n_r) =theta (n_m-nMstart+1,n_r)
! !           this%rhsvec_Mloc(n_m,n_r) =rhsvec (n_m,n_r)
!                                
!        enddo
!     enddo
...
!   do n_m=nMstart,nMstop
!   do n_r=1,n_r_max
!          print*,"this%theta_Mloc(n_m=,",n_m,"n_r=",n_r,")=",this%theta_Mloc(n_m,n_r)
! !          print*,"this%theta_Rloc(n_m=,",n_m,"n_r=",n_r,")=",this%theta_Rloc(n_m,n_r)
!          enddo
!   enddo 
    select case (piece)
    case (1)  ! Explicit piece

    call transp_m2r(m2r_fields, this%theta_Mloc, this%theta_Rloc)
      ...
      enddo
!       
!  print*,"Barrier f_eval avant mpi_barrier Key_Pizza =",Key_Pizza 
!     call mpi_barrier(Comm_Pizza, ierror)
! 
!  print*,"Barrier f_eval après mpi_barrier  Key_Pizza =",Key_Pizza 
!       
...
!     do n_r=1,n_r_max
!        do n_m=nMstart,nMstop
!           fvec(n_m-nMstart+1,n_r) = this%fvec_Mloc(n_m,n_r)
!                                
!        enddo
!     enddo
!                fvec =0.0! this%fvec_Mloc
               fvec =this%fvec_Mloc
! 
!  print*,"Barrier f_eval avant mpi_barrier Key_Pizza =",Key_Pizza 
!     call mpi_barrier(Comm_Pizza, ierror)
...
!        do n_m=nMstart,nMstop
!           this%fvec_Mloc(n_m,n_r) =fvec (n_m-nMstart+1,n_r)
!           this%theta_Mloc(n_m,n_r) =yvec (n_m-nMstart+1,n_r)
!           this%rhsvec_Mloc(n_m,n_r) =rhsvec (n_m-nMstart+1,n_r)
!                                
!        enddo
!     enddo

!           this%fvec_Mloc   = 0.0!fvec 
!           this%theta_Mloc  = 0.0!yvec 
!           this%rhsvec_Mloc = 0.0!rhsvec 
          this%fvec_Mloc   = fvec 
...
!        do n_m=nMstart,nMstop
!           fvec(n_m-nMstart+1,n_r) =this%fvec_Mloc (n_m,n_r)
!           yvec(n_m-nMstart+1,n_r) =this%theta_Mloc (n_m,n_r)
! !           this%rhsvec_Mloc(n_m,n_r) =rhsvec (n_m,n_r)
!                                
!        enddo
!     enddo
!           fvec =0.0!this%fvec_Mloc 
!           yvec =0.0!this%theta_Mloc   
          fvec =this%fvec_Mloc 
